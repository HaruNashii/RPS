...
use sdl3::{pixels::Color, rect::Rect, render::{Canvas, TextureCreator}, video::{Window, WindowContext}, ttf::Sdl3TtfContext, };
use crate::{sdl::sdl3_generators::{GenImage, GenText}, system::{input_handler::InputHandler, page_system::{Page, PageData}, scene_transition::{SceneTransition, TransitionType}}, AppState, Button, PersistentElements };
...
impl<'a, PageId: Copy + Eq, ButtonId: Copy + Eq> Renderer<'a, PageId, ButtonId> 
{
    ...
    pub fn render(&mut self, page_data: &PageData<PageId, ButtonId>, app_state: &mut AppState<PageId, ButtonId>, input_handler: &InputHandler<PageId, ButtonId>) 
    {
        ...
        if let Some(bg) = page.background_color { self.canvas.set_draw_color(bg); } else { self.canvas.set_draw_color(Color::RGB(0, 0, 0)); }
        self.canvas.clear();
    ...
    {
        // RECTS
        if let Some(rects) = &page.rects { for (color, (r, radius)) in rects { self.canvas.set_draw_color(*color); self.draw_rounded_box(r.x(), r.y(), r.width() as i32, r.height() as i32, *radius, *color,); } }
        // BUTTONS
        if let Some(buttons) = &page.buttons { for button in buttons { if button.enabled { let mut color = button.color; if let Some(button_selected) = input_handler.button_selected && let Some(amount_to_substract) = self.decrease_color_when_selected && button.id == button_selected {color = Color::RGB(button.color.r.saturating_sub(amount_to_substract.0), button.color.g.saturating_sub(amount_to_substract.1), button.color.b.saturating_sub(amount_to_substract.2)); }; self.canvas.set_draw_color(color); self.draw_rounded_box(button.rect.x(), button.rect.y(), button.rect.width() as i32, button.rect.height() as i32, button.radius, color); } } }
        // TEXTS
        ...
        // IMAGES
        if let Some(images) = &mut page.images { let mut image_data = (images, self.texture_creator); for (image_texture, image_rect) in image_data.generate_image() { let _ = self.canvas.copy(&image_texture,None,image_rect); } }
        ...
        if let Some(vec_page) = persistent_elements
        {
            for mut page in vec_page 
            {
                ...
                if let Some(buttons) = &page.buttons { for button in buttons { if button.enabled { let mut color = button.color; if let Some(button_selected) = input_handler.button_selected && let Some(amount_to_substract) = self.decrease_color_when_selected && button.id == button_selected {color = Color::RGB(button.color.r.saturating_sub(amount_to_substract.0), button.color.g.saturating_sub(amount_to_substract.1), button.color.b.saturating_sub(amount_to_substract.2)); }; self.canvas.set_draw_color(color); self.draw_rounded_box(button.rect.x(), button.rect.y(), button.rect.width() as i32, button.rect.height() as i32, button.radius, color); } } }
                // TEXTS
                if let Some(texts) = &mut page.texts { let mut requisites = (texts, self.texture_creator, self.ttf_context); for tuple in requisites.generate_text(self.font_path) { self.canvas.copy(&tuple.0, None, tuple.1).unwrap_or_else(|err| {println!("text creator gives an error \nerror: {}\n", err);}); } }
                // IMAGES
                if let Some(images) = &mut page.images { let mut image_data = (images, self.texture_creator); for (image_texture, image_rect) in image_data.generate_image() { let _ = self.canvas.copy(&image_texture,None,image_rect); } }
            }
        }
    ...
    {
        // RECTS
        if let Some(rects) = &page.rects { for (color, (r, radius)) in rects { self.canvas.set_draw_color(*color); self.draw_rounded_box(r.x() + direction_x, r.y() + direction_y, r.width() as i32, r.height() as i32, *radius, *color,); } }
        // BUTTONS
        if let Some(buttons) = &page.buttons { for button in buttons { if button.enabled { let mut color = button.color; if let Some(button_selected) = input_handler.button_selected && let Some(amount_to_substract) = self.decrease_color_when_selected && button.id == button_selected { color = Color::RGB(button.color.r.saturating_sub(amount_to_substract.0), button.color.g.saturating_sub(amount_to_substract.1), button.color.b.saturating_sub(amount_to_substract.2)); }; self.canvas.set_draw_color(color); self.draw_rounded_box(button.rect.x() + direction_x, button.rect.y() + direction_y, button.rect.width() as i32, button.rect.height() as i32, button.radius, color); } } }
        // TEXTS
    ...
    pub fn draw_rounded_box(&mut self, x: i32, y: i32, w: i32, h: i32, r: i32, color: Color)
    {
        ...
        for &dx in &[0,w-r]{ let _ = self.canvas.fill_rect(Rect::new(x+dx,y+r,r as u32,(h-2*r) as u32)); }
        let steps= r * 4;
        for i in 0..=steps
        {
    ...
    pub fn button_matches<T:Copy+Eq>(button: &Button<T>, a: T) -> bool { button.id == a }
